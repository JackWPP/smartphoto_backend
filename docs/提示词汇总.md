...
**版本标识：** `DETAIL_PLANNER_PROMPT_VERSION = "detail_planner_v4_prompt_first"`
...
中文站详情页规则：新增 panel 文案、copy_lines、panel_goal、copy_focus 必须是简体中文终稿候选；商品本体原有英文、型号、logo 可以保留，不要新增英文营销文案。
{_prompt_matrix_guardrail_text()}
...
### 1.7 详情页文案 Review Agent（已并回 Detail Planner）

当前默认主链已不再独立调用 `Detail Copy Review Agent`。  
原本 reviewer 负责补充的 `copy_focus / panel_goal / visual_truth_mode / origin_note` 已并回 `detail_planner` 一次产出，目的是减少默认 LLM 调用数并缩短详情页策略预览时延。
...
- 当 `domestic_chinese_copy=True` 时，追加：
  ...
  - "可见文案候选仅作为中文终稿语义参考，可改写但必须短而有信息密度，并保持简体中文：{visible_copy}"
...
| Block 键 | 内容 |
|----------|------|
| planning_context | 规划上下文（仅内部拼装使用，不再以带标签自然语言直接进入 final_prompt） |
...
| visual_contract | 归并后的视觉任务说明，替代直接暴露 `panel_type/layout_template/panel_goal` |
| copy_contract | 归并后的可见文案合同，替代直接暴露 `copy_focus/planning_context` |
...
- `detail_strategy_preview.panel_plan[*]`、`detail-pages/prompts/preview.prompts[*]`、`detail-pages/results.panels[*]`
  现在都会额外回传：
  - `display_module_title`
  - `display_module_kind`
  - `display_module_intent`
- 这组字段是用户侧的详情页模块真相源，用来替代旧的内部式标题，如：
  - `首屏亮点`
  - `核心概览`
  - `核心卖点`
  - `场景价值`
  ...
  - `细节参数`
  - `收尾总结`
- 旧 `panel_label/panel_type/panel_type_reason/planner_base/planner_source` 继续保留给后台和调试，不建议前端直接展示
