...
def compose_prompt(
    ...
    blocks = {
        ...
        "subject": _compose_subject_block(confirmed_copy, asset_role, prompt_plan),
        "composition": _compose_composition_block(plan, prompt_plan),
        "background": _compose_background_block(asset_role, plan, prompt_plan),
        "style": _compose_style_block(style, platform_strategy, plan, prompt_plan),
        "selling_points": _compose_selling_points_block(prompt_plan),
        "constraints": _compose_constraints_block(asset_role, prompt_plan),
        "instruction": _compose_instruction_block(instruction),
    }

    final_prompt = format_prompt_blocks(
        blocks,
...
def _compose_subject_block(confirmed_copy: dict[str, Any], asset_role: str, prompt_plan: dict[str, Any]) -> str:
    ...
    if asset_role == "white_bg":
        ...
        base = f"聚焦 {product_name} 的局部结构、材质纹理和工艺细节。"
    elif asset_role == "scene":
...
def _compose_background_block(asset_role: str, plan: dict[str, Any], prompt_plan: dict[str, Any]) -> str:
    ...
    if asset_role == "scene" or background_mode == "real_scene":
...
def _compose_selling_points_block(prompt_plan: dict[str, Any]) -> str:
...
def _compose_constraints_block(asset_role: str, prompt_plan: dict[str, Any]) -> str:
