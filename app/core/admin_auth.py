...
def issue_access_token(admin_user_id: str, username: str) -> str:
    ...
    payload = {
        ...
        "exp": int((now_utc() + timedelta(minutes=settings.admin_access_token_exp_minutes)).timestamp()),
        "scope": "admin",
    }
    return _encode_jwt(header, payload, settings.admin_jwt_secret)


def decode_access_token(token: str) -> dict[str, Any]:
    settings = get_settings()
    header, payload = _decode_jwt(token, settings.admin_jwt_secret)
    if header.get("alg") != "HS256":
        raise AppError("unauthorized", "invalid admin token", 401)
    exp = int(payload.get("exp") or 0)
    if exp <= int(now_utc().timestamp()):
        raise AppError("unauthorized", "admin token expired", 401)
    if payload.get("scope") != "admin":
        raise AppError("forbidden", "admin token scope invalid", 403)
    return payload
...
def issue_refresh_token() -> tuple[str, str, datetime]:
    ...
    expires_at = now_utc() + timedelta(days=settings.admin_refresh_token_exp_days)
...
def _decode_jwt(token: str, secret: str) -> tuple[dict[str, Any], dict[str, Any]]:
    try:
        ...
        raise AppError("unauthorized", "invalid admin token", 401) from exc
    signing_input = f"{header_segment}.{payload_segment}".encode("ascii")
