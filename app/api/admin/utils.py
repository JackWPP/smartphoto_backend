...
def serialize_job(job: JobModel) -> dict[str, Any]:
    ...
    return {
        ...
        "owner_kind": "user" if job.user_id else "guest",
        "owner_label": job.user_id or f"Guest {job.guest_id}",
...
def serialize_session(session: SessionModel) -> dict[str, Any]:
    return {
        ...
        "owner_kind": "user" if session.user_id else "guest",
        "owner_label": session.user_id or f"Guest {session.guest_id}",
