...
from app.api.admin.utils import paginate, serialize_rule_pack
...
from app.schemas.admin import (
    AdminRulePackDetailData,
    AdminRulePackListData,
    AdminRulePackMutationRequest,
    AdminRulePackUpdateRequest,
    AdminRulePackUpsertRequest,
)
...
def list_rule_packs(
    ...
    if q:
        q_like = f"%{q}%"
        query = query.filter((RulePackModel.name.ilike(q_like)) | (RulePackModel.rule_pack_key.ilike(q_like)))
    if not include_inactive:
        query = query.filter(RulePackModel.is_active.is_(True))
    sort_column = RulePackModel.updated_at
    if sort_by == "name":
        sort_column = RulePackModel.name
    elif sort_by == "created_at":
        sort_column = RulePackModel.created_at
    elif sort_by == "current_version_no":
        sort_column = RulePackModel.current_version_no
    query = query.order_by(sort_column.asc() if sort_order == "asc" else sort_column.desc())
    total = query.count()
    items = query.offset((page - 1) * page_size).limit(page_size).all()
    return success_response(
        {
            "items": [serialize_rule_pack(item, _latest_version(db, item.id)) for item in items],
            **paginate(total=total, page=page, page_size=page_size, sort_by=sort_by, sort_order=sort_order),
        }
    )


@router.get("/{rule_pack_id}", response_model=APIResponse[AdminRulePackDetailData], operation_id="adminGetRulePack", responses={**OPENAPI_ERROR_RESPONSES})
def get_rule_pack(rule_pack_id: str, db: Session = Depends(get_db), _admin_user=Depends(get_current_admin_user)) -> dict:
    ...
    return success_response(
        {
            "rule_pack": serialize_rule_pack(rule_pack, versions[0] if versions else None),
            "versions": [serialize_rule_pack(rule_pack, item)["version"] for item in versions if item is not None],
        }
    )
...
def create_rule_pack(
    req: AdminRulePackUpsertRequest,
    request: Request,
    db: Session = Depends(get_db),
    ...
    rule_pack = RulePackModel(
        name=req.name,
        asset_family=req.asset_family,
        platform_id=req.platform_id,
        rule_pack_key=req.rule_pack_key,
        is_system=False,
        is_active=True,
    ...
    append_admin_audit_log(
        ...
        action="rule_pack.create",
        module="rule_packs",
        risk_level="high",
        operator_note=req.operator_note,
        target_type="rule_pack",
        target_id=rule_pack.id,
        before_snapshot=None,
        after_snapshot=serialize_rule_pack(rule_pack, version),
...
def update_rule_pack(
    rule_pack_id: str,
    req: AdminRulePackUpdateRequest,
    request: Request,
    db: Session = Depends(get_db),
    admin_db: Session = Depends(get_admin_db),
    admin_user=Depends(get_current_admin_user),
    ...
    append_admin_audit_log(
        ...
        after_snapshot=serialize_rule_pack(rule_pack, latest),
        request_id=request_id_from_request(request),
    )
    db.commit()
    admin_db.commit()
...
def publish_rule_pack(
    rule_pack_id: str,
    req: AdminRulePackMutationRequest,
    request: Request,
    db: Session = Depends(get_db),
    admin_db: Session = Depends(get_admin_db),
    admin_user=Depends(get_current_admin_user),
...
def clone_rule_pack(
    ...
    if latest is None:
        raise AppError("invalid_request", "rule pack version not found", 404)
    clone = RulePackModel(
        name=f"{rule_pack.name} Copy",
        asset_family=rule_pack.asset_family,
        platform_id=rule_pack.platform_id,
        rule_pack_key=f"{rule_pack.rule_pack_key}_copy",
        is_system=False,
        is_active=True,
        current_version_no=1,
        created_by=admin_user.id,
    ...
    version = RulePackVersionModel(
        ...
        version_no=1,
        asset_family=clone.asset_family,
        platform_id=clone.platform_id,
        rule_pack_key=clone.rule_pack_key,
    ...
    append_admin_audit_log(
        admin_db,
        admin_user_id=admin_user.id,
        action="rule_pack.clone",
        module="rule_packs",
        risk_level="high",
        operator_note=req.operator_note,
        target_type="rule_pack",
        target_id=clone.id,
        before_snapshot=serialize_rule_pack(rule_pack, latest),
        after_snapshot=serialize_rule_pack(clone, version),
        request_id=request_id_from_request(request),
    )
    db.commit()
    admin_db.commit()
...
def archive_rule_pack(
    ...
    append_admin_audit_log(
        ...
        target_id=rule_pack.id,
        before_snapshot=before,
        after_snapshot=serialize_rule_pack(rule_pack, _latest_version(db, rule_pack.id)),
        request_id=request_id_from_request(request),
    )
    db.commit()
    admin_db.commit()
